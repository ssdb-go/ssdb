...
func Struct(dst interface{}) (StructValue, error) {
	...
	if v.Kind() != reflect.Ptr || v.IsNil() {
		return StructValue{}, fmt.Errorf("ssdb.Scan(non-pointer %T)", dst)
	}

	v = v.Elem()
	if v.Kind() != reflect.Struct {
		return StructValue{}, fmt.Errorf("ssdb.Scan(non-struct %T)", dst)
	}
...
// Scan scans the results from a key-value ssdb map result set to a destination struct.
// The ssdb keys are matched to the struct's field with the `ssdb` tag.
...
func decodeUnsupported(v reflect.Value, s string) error {
	return fmt.Errorf("ssdb.Scan(unsupported %s)", v.Type())
}
