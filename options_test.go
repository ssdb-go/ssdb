...
func TestParseURL(t *testing.T) {
	cases := []struct {
		...
		{
			url: "ssdb://localhost:123/1",
			o:   &Options{Addr: "localhost:123", DB: 1},
		}, {
			url: "ssdb://localhost:123",
			o:   &Options{Addr: "localhost:123"},
		}, {
			url: "ssdb://localhost/1",
			o:   &Options{Addr: "localhost:8888", DB: 1},
		}, {
			url: "ssdb://12345",
			o:   &Options{Addr: "12345:8888"},
		}, {
			url: "ssdbs://localhost:123",
			o:   &Options{Addr: "localhost:123", TLSConfig: &tls.Config{ /* no deep comparison */ }},
		}, {
			url: "ssdb://:bar@localhost:123",
			o:   &Options{Addr: "localhost:123", Password: "bar"},
		}, {
			url: "ssdb://foo@localhost:123",
			o:   &Options{Addr: "localhost:123", Username: "foo"},
		}, {
			url: "ssdb://foo:bar@localhost:123",
			o:   &Options{Addr: "localhost:123", Username: "foo", Password: "bar"},
		}, {
			// multiple params
			url: "ssdb://localhost:123/?db=2&read_timeout=2&pool_fifo=true",
			o:   &Options{Addr: "localhost:123", DB: 2, ReadTimeout: 2 * time.Second, PoolFIFO: true},
		}, {
			// special case handling for disabled timeouts
			url: "ssdb://localhost:123/?db=2&idle_timeout=0",
			o:   &Options{Addr: "localhost:123", DB: 2, ConnMaxIdleTime: -1},
		}, {
			// negative values disable timeouts as well
			url: "ssdb://localhost:123/?db=2&idle_timeout=-1",
			o:   &Options{Addr: "localhost:123", DB: 2, ConnMaxIdleTime: -1},
		}, {
			// absent timeout values will use defaults
			url: "ssdb://localhost:123/?db=2&idle_timeout=",
			o:   &Options{Addr: "localhost:123", DB: 2, ConnMaxIdleTime: 0},
		}, {
			url: "ssdb://localhost:123/?db=2&idle_timeout", // missing "=" at the end
			o:   &Options{Addr: "localhost:123", DB: 2, ConnMaxIdleTime: 0},
		}, {
			url: "unix:///tmp/ssdb.sock",
			o:   &Options{Addr: "/tmp/ssdb.sock"},
		}, {
			url: "unix://foo:bar@/tmp/ssdb.sock",
			o:   &Options{Addr: "/tmp/ssdb.sock", Username: "foo", Password: "bar"},
		}, {
			url: "unix://foo:bar@/tmp/ssdb.sock?db=3",
			o:   &Options{Addr: "/tmp/ssdb.sock", Username: "foo", Password: "bar", DB: 3},
		}, {
			// invalid db format
			url: "unix://foo:bar@/tmp/ssdb.sock?db=test",
			err: errors.New(`ssdb: invalid database number: strconv.Atoi: parsing "test": invalid syntax`),
		}, {
			// invalid int value
			url: "ssdb://localhost/?pool_size=five",
			err: errors.New(`ssdb: invalid pool_size number: strconv.Atoi: parsing "five": invalid syntax`),
		}, {
			// invalid bool value
			url: "ssdb://localhost/?pool_fifo=yes",
			err: errors.New(`ssdb: invalid pool_fifo boolean: expected true/false/1/0 or an empty string, got "yes"`),
		}, {
			// it returns first error
			url: "ssdb://localhost/?db=foo&pool_size=five",
			err: errors.New(`ssdb: invalid database number: strconv.Atoi: parsing "foo": invalid syntax`),
		}, {
			url: "ssdb://localhost/?abc=123",
			err: errors.New("ssdb: unexpected option: abc"),
		}, {
			url: "ssdb://foo@localhost/?username=bar",
			err: errors.New("ssdb: unexpected option: username"),
		}, {
			url: "ssdb://localhost/?wrte_timout=10s&abc=123",
			err: errors.New("ssdb: unexpected option: abc, wrte_timout"),
		}, {
			url: "http://google.com",
			err: errors.New("ssdb: invalid URL scheme: http"),
		}, {
			url: "ssdb://localhost/1/2/3/4",
			err: errors.New("ssdb: invalid URL path: /1/2/3/4"),
		}, {
			url: "12345",
			err: errors.New("ssdb: invalid URL scheme: "),
		}, {
			url: "ssdb://localhost/iamadatabase",
			err: errors.New(`ssdb: invalid database number: "iamadatabase"`),
		},
	}
